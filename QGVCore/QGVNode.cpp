...
{
  pointf pt = ND_coord(_node->node());
  return QPointF(pt.x, pt.y);
}

QPointF QGVNode::calcPos()
{
  ...
  return QGVCore::centerToOrigin(QGVCore::toPoint(ND_coord(_node->node()), gheight), width, height);
}

qreal QGVNode::gHeight()
{
  return QGVCore::graphHeight(_scene->_graph->graph());
}
