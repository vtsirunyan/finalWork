...
void QGVEdge::paint(QPainter * painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    */
    painter->drawText(_label_rect, Qt::AlignHCenter, _label);
//    painter->setPen("blue");
//    painter->drawRect(_path.boundingRect());
//    painter->setPen("red");
...
void QGVEdge::updateLayout()
{
    ...
    //Edge label
    textlabel_t *xlabel = ED_label(_edge->edge());
    if(xlabel)
    {
        _label = xlabel->text;
...
void QGVEdge::setTo(QGVNode *to)
{
  m_to = to;
}

void QGVEdge::setFrom(QGVNode *from)
{
  m_from = from;
}

QGVNode *QGVEdge::getTo() const
{
  return m_to;
}

QGVNode *QGVEdge::getFrom() const
{
  return m_from;
}
